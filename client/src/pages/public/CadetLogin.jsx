...
export default function CadetLogin() {
  ...
  const { loginCadet, loginWithGoogle } = useAuth();
  const navigate = useNavigate();

  const handleAutoFill = (e) => {
    if (e) e.preventDefault();
    setForm({ email: 'cadet.rahul@ncc.gov.in', password: 'Password@123' });
    setError('');
  };

  const handleQuickDemoLogin = async (e) => {
    if (e) e.preventDefault();
    const demoEmail = 'cadet.rahul@ncc.gov.in';
    const demoPassword = 'Password@123';
    setForm({ email: demoEmail, password: demoPassword });
    setError('');
    setLoading(true);
    const result = await loginCadet(demoEmail, demoPassword);
    setLoading(false);
    if (result.success) {
      toast.success('Welcome back, Cadet Rahul!');
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const result = await loginCadet(form.email.trim(), form.password);
    setLoading(false);
      ...
      <div className="hidden lg:flex flex-col justify-between w-[45%] hero-gradient p-12 relative overflow-hidden">
        ...
        <div>
          ...
          <p className="text-gray-300 leading-relaxed text-base mb-8">
            Log in with your email address to access your NCC profile, attendance, training records and more.
          </p>
          <div className="space-y-3">
            {['View your attendance record', 'Track training progress', 'Apply for leave', 'Check camp registrations', 'View certificates & achievements'].map(item => (
              <div key={item} className="flex items-center gap-3 text-gray-300 text-sm">
                <div className="w-5 h-5 rounded-full bg-sky/20 flex items-center justify-center flex-shrink-0">
        ...
        <div className="w-full max-w-md">
          <div className="card animate-fade-in">
            <div className="text-center mb-7">
              ...
              <h1 className="text-2xl font-black text-navy-500 mb-1">Cadet Login</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-5" noValidate>
              ...
              <div className="flex justify-end">
                <button type="button" className="text-xs text-sky font-medium hover:text-sky-500 transition-colors">
                  Forgot Password?
                </button>
              </div>
            ...
            {/* Demo credentials */}
            <div className="mt-5 p-4 bg-sky-50 border border-sky-100 rounded-lg">
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs font-semibold text-sky-700">Demo Cadet Account</p>
                <button
                  type="button"
                  onClick={handleAutoFill}
                  className="text-xs font-medium text-sky-600 hover:text-sky-800 underline transition-colors"
                >
                  Auto-fill
                </button>
              </div>
              <div className="text-xs text-gray-600 space-y-1 mb-3">
                <p>Email: <span className="font-mono font-semibold text-gray-800">cadet.rahul@ncc.gov.in</span></p>
                <p>Password: <span className="font-mono font-semibold text-gray-800">Password@123</span></p>
              </div>
              <button
                type="button"
                onClick={handleQuickDemoLogin}
                className="w-full py-1.5 px-3 bg-sky-600 hover:bg-sky-700 text-white rounded text-xs font-semibold shadow transition-all flex items-center justify-center gap-1.5"
              >
          ...
          <p className="text-center text-sm text-gray-500 mt-5">
            Are you an Officer?{' '}
            <Link to="/login/officer" className="text-primary font-semibold hover:text-primary-600 transition-colors">
              Login as Officer
            </Link>
          </p>
        </div>
